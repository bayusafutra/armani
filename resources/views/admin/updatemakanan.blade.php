    ...
    <div class="card-body">
        <form action="/makananupdate" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select id="province_id" class="form-select" name="province_id">
                    @foreach ($provinsi as $prov )
                        @if (old('province_id', $makanan[0]->province_id) == $prov->id)
            ...
            <div class="form-floating mb-3">
                ...
                <input type="text" class="form-control @error('slug') is-invalid @enderror" style="background-color: #2A3038" name="slug" id="slug" required value="{{ old('slug', $makanan[0]->slug) }}"/>
                @error('slug')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="form-floating mb-3">
                ...
                @enderror
                <input id="deskripsi_makanan" type="hidden" name="deskripsi_makanan" value="{{ old('deskripsi_makanan', $makanan[0]->deskripsi_makanan) }}">
                <trix-editor input="deskripsi_makanan"></trix-editor>
            </div>

            <div class="mb-3">
                <label for="gambar">Gambar</label>
                <input type="hidden" name="oldImage" value="{{ $makanan[0]->gambar }}">

                @if (strlen($makanan[0]->gambar)>50)
                    <img src="{{ asset('storage/'.$makanan[0]->gambar) }}" class="img-preview img-fluid mb-3 col-sm-5 d-block">
                @else
                    <img src="/img/makanan/{{ $makanan[0]->gambar }}" class="img-preview img-fluid mb-3 col-sm-5 d-block">
                @endif
